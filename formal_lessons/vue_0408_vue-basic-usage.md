...
- MVC（Model View Controller）（模型-视图-控制器） => （对整体）设计认知 （软件设计规范）

- 不同范畴（不同维度）具有不同含义：

  **观点 1**——jQuery 是 MVC（纯前端角度理解 MVC 所处理的能力）：
  ...
  **观点 2**——jQuery 是前端框架，加上后台接口、中转层 controller 加起来才是 MVC 的东西（上升维度：整个应用、前后端加在一起的层级，应用整合、应用架构角度理解 MVC）
...
- MVVM（Model-View-ViewModel）（数据、状态-视图-数据视图绑定层）（对主流框架有使用经验和了解）
- ViewModel 是 MVVM 所特有的数据视图绑定层。每个框架的实现用的发布订阅。
- ViewModel 所代表的思想：

  1. 数据会绑定在 viewModel 层，并且会直接驱动数据到页面的渲染 => 自动 trigger 的功能（视图增加的第一层自动化）：

     - 自动 trigger 的功能：MVVM 和 jQuery 最大区别：jQuery 的数据都需要手动调用$dom 的名字.value = xxx，把数据替换掉现有的 dom 的数据。
     - 数据变化 + 视图绑定数据变量 => 视图数据变量，自动发生变化

  2. （数据 -> 视图，数据变化到视图变化：vue-{{}}，react-setState、小程序-setData）  
...
- ViewModel 的作用（与 MVC 的差别就在于 ViewModel）：
...
1. 【vue2】：
   ...
   - bC：new Vue() - 初始化类、挂载属性（新建 vue 实例，实例挂载功能）
   - c：
     1. merge options 的操作：
